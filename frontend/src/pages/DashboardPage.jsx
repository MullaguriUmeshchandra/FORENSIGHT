...
import { FolderArchive, CheckCircle, AlertTriangle, Lightbulb, FileText, Sparkles } from 'lucide-react';

export const DashboardPage = () => {
  const { currentCase, refreshKey, initialLoaded, loadDemoCase } = useCase();
  ...
  if (!initialLoaded || (loading && !summary && currentCase)) {
  ...
  if (initialLoaded && !currentCase) {
    return (
      <EmptyState
        title="No Active Forensic Case"
        description="Select an existing case from the topbar or load the pre-built CASE-001 scenario to begin."
        actionLabel="Load Demonstration Case"
